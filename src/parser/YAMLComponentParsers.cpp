...
void parse_Composite(Composite* component,
  ...
  if (node["components"]) {
    if (node["components"].IsSequence()) {
      for (YAML::const_iterator it = node["components"].begin(); it != node["components"].end();
           ++it) {
...
void parse_Grid(Grid<GridImpl>* component,
                const YAML::Node& node,
                const std::set<std::string>& in,
                YAMLAbstractParser* parser) {
...
Component* create_LayeredModel(const YAML::Node& node,
  ...
  if (map->componentCount() > 0) {
    ...
    // NOTE: suppliedParameters yields the actual output variables of a Composite with sub
    // Components (out only contains the pre-post-component-application parameters)
