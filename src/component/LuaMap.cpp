...
static std::string loadCodeLmathX() { return ""; }
} // namespace
#endif
...
double getField(lua_State* L, const std::string& key) {
  ...
  lua_gettable(L, -2);
  if (!lua_isnumber(L, -1)) {
    std::cerr << "Tried key " << key << " but it did not return a number." << std::endl;
    std::abort();
  }
...
void LuaMap::executeLuaFunction(const Matrix<double>& x, Matrix<double>& y) {
#ifdef _OPENMP
...
#pragma omp parallel
#endif
  {
    ...
    if (luaState == nullptr) {
      ...
      if (status != 0) {
        std::cerr << "Couldn't load script: " << lua_tostring(luaState, -1);
        std::abort();
      }
    }
...
#pragma omp for
    ...
    for (unsigned row = 0; row < y.rows(); ++row) {
      ...
      if (lua_pcall(luaState, 1, 1, 0) != 0) {
        std::cerr << "Error running function f " << lua_tostring(luaState, -1)
                  << " :::: f given by: " << function << std::endl;
        std::abort();
      }
