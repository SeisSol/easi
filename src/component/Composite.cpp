...
void Composite::evaluate(Query& query, ResultAdapter& result) {
  ...
  if (nComponents == 0) {
    ...
    unsigned col = 0;
    for (const std::string& param : out()) {
...
void Composite::couldNotFindModelError(int group, const Slice<double>& y) {
  std::stringstream ss;
...
std::string Composite::addMissingParameters(const std::string& what_arg,
...
std::set<std::string> Composite::suppliedParameters() {
  ...
  return std::accumulate(
      begin(), end(), std::set<std::string>(), [](std::set<std::string>& a, Component*& b) {
        std::set<std::string> supplied;
        auto c = b->suppliedParameters();
        std::set_union(
            a.begin(), a.end(), c.begin(), c.end(), std::inserter(supplied, supplied.begin()));
        return supplied;
      });
}
