...
easi::Composite::~Composite() {
  std::for_each(begin(), end(), [](Component*& component) { delete component; });
}

void easi::Composite::add(Component* component) {
  if (out() != component->in()) {
...
void easi::Composite::evaluate(Query& query, ResultAdapter& result) {
  // Map dimensions
...
void easi::Composite::couldNotFindModelError(int group, Slice<double> const& y) {
  std::stringstream ss;
...
std::string easi::Composite::addMissingParameters(std::string const& what_arg, std::set<std::string> expected, std::set<std::string> supplied) {
...
std::set<std::string> easi::Composite::suppliedParameters() {
