...
void LayeredModelBuilder::setInterpolationType(const std::string& interpolationType) {
...
void LayeredModelBuilder::setNodes(const Nodes& nodes) {
...
Component* LayeredModelBuilder::createModel(const std::optional<Nodes::iterator::value_type>& lower,
  ...
  const double lowerLimit =
      lower.has_value() ? lower.value().first : -std::numeric_limits<double>::infinity();
  const double upperLimit =
      upper.has_value() ? upper.value().first : std::numeric_limits<double>::infinity();
  ...
  if (interpolationType == Linear) {
    for (unsigned i = 0; i < nParams; ++i) {
      coeffs[m_parameters[i]][0] = (lower.value().second[i] - upper.value().second[i]) /
                                   (lower.value().first - upper.value().first);
      coeffs[m_parameters[i]][1] = (-upper.value().first * lower.value().second[i] +
                                    lower.value().first * upper.value().second[i]) /
                                   (lower.value().first - upper.value().first);
    }
