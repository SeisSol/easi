...
YAMLParser::YAMLParser(const std::set<std::string>& variables, AsagiReader* externalAsagiReader)
    : m_in(variables), m_asagiReader(externalAsagiReader),
      m_externalAsagiReader(externalAsagiReader != nullptr) {
...
void YAMLParser::registerType(const std::string& tag, std::function<CreateFunction> create) {
  m_creators[tag] = std::move(create);
}

Component* YAMLParser::parse(const std::string& fileName) {
  Component* root;
  try {
...
Component* YAMLParser::parse(const YAML::Node& node, const std::set<std::string>& in) {
