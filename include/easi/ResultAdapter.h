...
class ResultAdapter {
  ...
  virtual void set(const std::string& parameter,
                   const Vector<unsigned>& index,
                   const Slice<double>& value) = 0;
  virtual bool isSubset(const std::set<std::string>& parameters) const = 0;
  virtual ResultAdapter* subsetAdapter(const std::set<std::string>& subset) = 0;
...
class ArrayOfStructsAdapter : public ResultAdapter {
  ...
  void addBindingPoint(const std::string& parameter, double T::* pointerToMember) {
    m_bindingPoint[parameter] = m_parameter.size();
    m_parameter.push_back(pointerToMember);
  }

  int bindingPoint(const std::string& parameter) const {
    auto it = m_bindingPoint.find(parameter);
    return (it != m_bindingPoint.end()) ? it->second : -1;
  }

  virtual bool isSubset(const std::set<std::string>& parameters) const {
    for (const auto& kv : m_bindingPoint) {
  ...
  virtual void
      set(const std::string& parameter, const Vector<unsigned>& index, const Slice<double>& value) {
  ...
  virtual ResultAdapter* subsetAdapter(const std::set<std::string>& subset) {
...
class ArraysAdapter : public ResultAdapter {
  ...
  void addBindingPoint(const std::string& parameter, Float* array, unsigned stride = 1) {
    m_bindingPoint[parameter] = m_arrays.size();
    m_arrays.push_back(array);
    m_strides.push_back(stride);
  }

  virtual int bindingPoint(const std::string& parameter) const {
    auto it = m_bindingPoint.find(parameter);
    return (it != m_bindingPoint.end()) ? it->second : -1;
  }
