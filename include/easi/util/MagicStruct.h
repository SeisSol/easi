...
template <typename T>
static const std::string* get_member_names();
template <typename T>
static int get_binding_point(const std::string& member);
template <typename T>
constexpr static int get_number_of_members();
template <typename T>
static double T::* get_pointer_to_member(int bp);

template <typename T>
static std::set<std::string> memberNamesToSet() {
  const std::string* begin = get_member_names<T>();
  const std::string* end = begin + get_number_of_members<T>();
...
#define SELF_AWARE_STRUCT(STRUCT, ...)                                                             \
  namespace easi {                                                                                 \
  template <>                                                                                      \
  std::string const* get_member_names<STRUCT>() {                                                  \
    static std::string const names[] = {                                                           \
        MAGIC_FOR_EACH_COMMA_SEPARATED(SAS_TOSTRING, STRUCT, __VA_ARGS__)};                        \
    return names;                                                                                  \
  }                                                                                                \
  template <>                                                                                      \
  int get_binding_point<STRUCT>(std::string const& field) {                                        \
  ...
  template <>                                                                                      \
  constexpr int get_number_of_members<STRUCT>() {                                                  \
    return MAGIC_NARG(__VA_ARGS__);                                                                \
  }                                                                                                \
  template <>                                                                                      \
  double STRUCT::* get_pointer_to_member<STRUCT>(int bp) {                                         \
